...
// ── PAGE COUNTER ──────────────────────────────────────────────────────────────
// Counts how many scan cycles have completed for the current notebook session.
// Incremented at the end of each runScanCycle().
// Reset to 0 when the conveyor starts running again (new notebook or session end).
int pageCount = 0;
// ─────────────────────────────────────────────────────────────────────────────
...
void runScanCycle() {
  ...
  // ── Step 6: Increment page counter and report ─────────────────────────
  pageCount++;
...
void loop() {
  ...
  if (notebookState == HIGH && lastPageState == HIGH) {
    ...
    if (!isConveyorMoving) {
      ...
      // ── Reset page counter when conveyor resumes ───────────────────────
      // Triggered once per transition (isConveyorMoving flips from false→true).
      // Guard (pageCount > 0) avoids a spurious "0 pages" message on an
      // empty-belt run where no notebook was ever scanned.
      if (pageCount > 0) {
